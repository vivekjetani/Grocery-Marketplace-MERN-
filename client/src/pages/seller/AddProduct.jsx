...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (data.success) { toast.success(data.message); setName(""); setDescription(""); setCategory(""); setPrice(""); setOfferPrice(""); setFiles([]); }
  ...
  return (
    <div className="py-10 flex flex-col justify-between bg-white dark:bg-slate-950">
      <form onSubmit={handleSubmit} className="md:p-10 p-4 space-y-5 max-w-lg">
        <div>
          <p className="text-base font-medium text-slate-900 dark:text-white">Product Image</p>
          <div className="flex flex-wrap items-center gap-3 mt-2">
            {Array(4).fill("").map((_, index) => (
              <label key={index} htmlFor={`image${index}`}>
                ...
                <img className="max-w-24 cursor-pointer rounded-lg border border-slate-200 dark:border-slate-600" src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt="Upload area" width={100} height={100} />
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="w-full flex flex-col gap-1">
          ...
          <select id="category" value={category} onChange={(e) => setCategory(e.target.value)} className="outline-none md:py-2.5 py-2 px-3 rounded border border-slate-500/40 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100">
            <option value="">Select Category</option>
            {categories.map((category, index) => (<option value={category.path} key={index}>{category.path}</option>))}
          </select>
        </div>
        <div className="flex items-center gap-5 flex-wrap">
          <div className="flex-1 flex flex-col gap-1 w-32">
            <label className="text-base font-medium text-slate-900 dark:text-white" htmlFor="product-price">Product Price</label>
