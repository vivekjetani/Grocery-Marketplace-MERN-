  ...
  return (
    <div className="flex-1 py-10 flex flex-col justify-between">
      <div className="w-full md:p-10 p-4">
        <h2 className="pb-4 text-lg font-medium text-slate-900 dark:text-white">All Products</h2>
        <div className="flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white dark:bg-slate-800 border border-slate-500/20 dark:border-slate-700">
          <table className="md:table-auto table-fixed w-full overflow-hidden">
            ...
            <tbody className="text-sm text-slate-500 dark:text-slate-400">
              {products.map((product) => (
                <tr key={product._id} className="border-t border-slate-500/20 dark:border-slate-700">
                  <td className="md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3 truncate">
                    <div className="border border-slate-300 dark:border-slate-600 rounded p-2">
                      <img src={`${import.meta.env.VITE_BACKEND_URL}/images/${product.image[0]}`} alt={product.name} className="w-16" />
                    </div>
